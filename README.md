...
- 2018.12-2019.2: 在CVTE中央研究院 优化完线上问答机器人(微信公众号:希沃学院)的性能(主要是:用户反馈的解决率).主要是在做检索式的问答匹配.由于数据类别极多,极其不均匀.而且觉得是个度量问题(其实是单做分类问题来做的时候 效果极差)，将模型转化成一个聚类模型，其中特征抽取用深度学习模型来做。目前已将模型的top3从0.46提升到 0.76.
